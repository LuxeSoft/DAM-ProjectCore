...
import messages
import settings
from db.models import Player, PlayerToken
...
class ResourceCreateUserToken(DAMCoreResource):
    def on_post(self, req, resp, *args, **kwargs):
        ...
        current_user = self.db_session.query(Player).filter(Player.email == auth_username).one_or_none()

        if current_user is None:
            current_user = self.db_session.query(Player).filter(Player.username == auth_username).one_or_none()
...
class ResourceDeleteUserToken(DAMCoreResource):
    def on_post(self, req, resp, *args, **kwargs):
        ...
        selected_token = self.db_session.query(PlayerToken).filter(PlayerToken.token == selected_token_string).one_or_none()
